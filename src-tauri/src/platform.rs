...
pub fn detect_browsers() -> Vec<BrowserConfig> {
    ...
    if let Some(browser) = check_browser("chrome", "Google Chrome", &chrome_paths.iter().map(|s|s.as_str()).collect::<Vec<_>>()) {
...
pub fn detect_browsers() -> Vec<BrowserConfig> {
    ...
    let chrome_paths = [format!("{}/Google Chrome.app/Contents/MacOS/Google Chrome", app_dir)];
    if let Some(browser) = check_browser("chrome", "Google Chrome", &chrome_paths.iter().map(|s|s.as_str()).collect::<Vec<_>>()) {
        browsers.push(browser);
    }
    
    let edge_paths = [format!("{}/Microsoft Edge.app/Contents/MacOS/Microsoft Edge", app_dir)];
    if let Some(browser) = check_browser("edge", "Microsoft Edge", &edge_paths.iter().map(|s|s.as_str()).collect::<Vec<_>>()) {
        browsers.push(browser);
    }
